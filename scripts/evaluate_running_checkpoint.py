...
class Config:
    ...
    upright_peak_tilt_deg: float = 45.0
    seed: int = 123
...
def _camera_axes(quat: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Return the camera site's local +X forward and +Z up axes in world space."""
    w, x, y, z = quat.unbind(dim=-1)
    forward = torch.stack(
        (
...
def main(cfg: Config) -> None:
    ...
    raw_env = ManagerBasedRlEnv(
        cfg=env_cfg,
        device=device,
        render_mode="rgb_array" if cfg.record_dir else None,
    )
    base_env = raw_env
    if cfg.record_dir:
        ...
        base_env = VideoRecorder(
            raw_env,
            video_folder=Path(cfg.record_dir),
    ...
    head_site_ids, _ = robot.find_sites("head_camera")
    if len(head_site_ids) != 1:
        raise RuntimeError(f"expected one head_camera site, found {head_site_ids}")
    head_site_id = head_site_ids[0]
    ...
    peak_forward_speed = torch.full_like(speed_sum, -torch.inf)
    upright_peak_forward_speed = torch.full_like(speed_sum, -torch.inf)
    head_forward, head_up = _camera_axes(robot.data.site_quat_w[:, head_site_id])
    previous_head_forward = head_forward.clone()
    previous_head_up = head_up.clone()
    head_horizon_tilt_samples: list[torch.Tensor] = []
    head_up_error_samples: list[torch.Tensor] = []
    head_angular_rate_samples: list[torch.Tensor] = []
    neck_action_rate_samples: list[torch.Tensor] = []
    leg_action_rate_samples: list[torch.Tensor] = []
    ...
    for step in range(num_steps):
        ...
        if previous_actions is not None:
            action_rate = (actions - previous_actions).abs() / raw_env.step_dt
        ...
        head_forward, head_up = _camera_axes(robot.data.site_quat_w[:, head_site_id])
        forward_delta = torch.acos(
            torch.clamp((head_forward * previous_head_forward).sum(dim=-1), -1.0, 1.0)
        )
        up_delta = torch.acos(
            torch.clamp((head_up * previous_head_up).sum(dim=-1), -1.0, 1.0)
        )
        head_angular_rate = 0.5 * (forward_delta + up_delta) / raw_env.step_dt
        previous_head_forward = head_forward.clone()
        previous_head_up = head_up.clone()
        if step >= warmup_steps:
            ...
            peak_forward_speed = torch.maximum(peak_forward_speed, forward_speed)
            upright_for_peak = tilt < math.radians(cfg.upright_peak_tilt_deg)
            upright_peak_forward_speed = torch.maximum(
                upright_peak_forward_speed,
                torch.where(
                    upright_for_peak,
                    forward_speed,
                    torch.full_like(forward_speed, -torch.inf),
                ),
            )
            ...
            head_horizon_tilt = torch.asin(
                torch.clamp(head_forward[:, 2].abs(), 0.0, 1.0)
            )
            head_up_error = torch.acos(torch.clamp(head_up[:, 2], -1.0, 1.0))
            head_horizon_tilt_samples.append(torch.rad2deg(head_horizon_tilt[valid]))
            head_up_error_samples.append(torch.rad2deg(head_up_error[valid]))
            head_angular_rate_samples.append(torch.rad2deg(head_angular_rate[valid]))
            if action_rate is not None:
                neck_action_rate_samples.append(action_rate[valid, 5:9].flatten())
                leg_action_rate_samples.append(
                    torch.cat((action_rate[valid, :5], action_rate[valid, 9:]), dim=1)
                    .flatten()
                )
    ...
    head_horizon_tilt = torch.cat(head_horizon_tilt_samples)
    head_up_error = torch.cat(head_up_error_samples)
    head_angular_rate = torch.cat(head_angular_rate_samples)
    neck_action_rate = torch.cat(neck_action_rate_samples)
    leg_action_rate = torch.cat(leg_action_rate_samples)
    result = {
        ...
        "instantaneous_forward_speed_mps": {
            "max": float(peak_forward_speed.max()),
            "p99": _quantile(peak_forward_speed, 0.99),
            "median_of_env_peaks": _quantile(peak_forward_speed, 0.50),
        },
        "upright_instantaneous_forward_speed_mps": {
            "tilt_gate_deg": cfg.upright_peak_tilt_deg,
            "max": float(upright_peak_forward_speed.max()),
            "p99": _quantile(upright_peak_forward_speed, 0.99),
            "median_of_env_peaks": _quantile(upright_peak_forward_speed, 0.50),
        },
        ...
        "head_camera_horizon_tilt_deg": _distribution(head_horizon_tilt),
        "head_camera_up_error_deg": _distribution(head_up_error),
        "head_camera_angular_rate_deg_s": _distribution(head_angular_rate),
        "neck_action_rate_units_s": _distribution(neck_action_rate),
        "leg_action_rate_units_s": _distribution(leg_action_rate),
    }
