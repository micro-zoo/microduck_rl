...
# A larger synchronous rollout already contributes proportionally more samples
# per PPO iteration.  Set this to num_envs / 512 to retain the same sample-count
# curriculum instead of multiplying the total training budget.  The default is
# one so existing 512-env recipes are byte-for-byte unchanged.
RUNNING_CURRICULUM_DIVISOR = int(
    os.environ.get("MICRODUCK_RUNNING_CURRICULUM_DIVISOR", "1")
)
if RUNNING_CURRICULUM_DIVISOR <= 0:
    raise ValueError("running curriculum divisor must be positive")
...
def _running_curriculum_step(iteration: int) -> int:
    """Map a 512-env recipe iteration to its sample-equivalent step."""
    return round(iteration / RUNNING_CURRICULUM_DIVISOR) * NUM_STEPS_PER_ENV


_RUNNING_BASE_SPEED_STAGES = (
    {"step": _running_curriculum_step(0), "min_speed": 0.20, "max_speed": 0.45},
    {"step": _running_curriculum_step(1000), "min_speed": 0.30, "max_speed": 0.55},
    {"step": _running_curriculum_step(2000), "min_speed": 0.40, "max_speed": 0.65},
    {"step": _running_curriculum_step(3000), "min_speed": 0.50, "max_speed": 0.75},
    {"step": _running_curriculum_step(4000), "min_speed": 0.60, "max_speed": 0.85},
    {"step": _running_curriculum_step(5000), "min_speed": 0.70, "max_speed": 0.95},
    {"step": _running_curriculum_step(6000), "min_speed": 0.90, "max_speed": 1.20},
)
...
def _running_speed_stages(
    ...
    stages = _RUNNING_BASE_SPEED_STAGES + (
        {
            "step": _running_curriculum_step(6750),
            "min_speed": 0.95,
            "max_speed": intermediate_1,
        },
        {
            "step": _running_curriculum_step(7250),
            "min_speed": 1.05,
            "max_speed": intermediate_2,
        },
        {
            "step": _running_curriculum_step(7750),
            "min_speed": 1.15,
    ...
    return stages + tuple(
        {
            "step": _running_curriculum_step(
                8750 + index * high_speed_stage_interval
            ),
...
def _apply_running_robustness(
    ...
    *,
    ...
    if not 0.0 < friction_range[0] <= friction_range[1]:
        raise ValueError(
            "running robustness friction range must be positive and ordered"
        )
    if not 0.0 < push_interval_s[0] <= push_interval_s[1]:
        raise ValueError(
            "running robustness push interval must be positive and ordered"
        )
        ...
        cfg.curriculum["action_rate_weight"] = CurriculumTermCfg(
            ...
            params={
                ...
                "weight_stages": [
                    {
                        "step": _running_curriculum_step(
                            0 - RUNNING_CURRICULUM_START_ITERATION
                        ),
                        "weight": -0.02,
                    },
                    {
                        "step": _running_curriculum_step(
                            2500 - RUNNING_CURRICULUM_START_ITERATION
                        ),
                        "weight": -0.05,
                    },
                    {
                        "step": _running_curriculum_step(
                            4000 - RUNNING_CURRICULUM_START_ITERATION
                        ),
                        "weight": RUNNING_FINAL_ACTION_RATE_WEIGHT,
                    },
                    {
                        "step": _running_curriculum_step(
                            5500 - RUNNING_CURRICULUM_START_ITERATION
                        ),
                        "weight": RUNNING_FINAL_ACTION_RATE_WEIGHT,
                    },
                ],
